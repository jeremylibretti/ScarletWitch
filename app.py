...
def main():
    ...
    # Open initial webcam capture in foreground
    # ret, image = cap.read()
    # if ret:
    #     image = cv.flip(image, 1)  # Mirror display
    #     cv.imshow('Hand Gesture Recognition', image)
    #     # print(cv.getWindowProperty('Hand Gesture Recognition', cv.WND_PROP_TOPMOST))
    ...
    #     # cv.setWindowProperty('Hand Gesture Recognition',cv.WND_PROP_FULLSCREEN,cv.WINDOW_NORMAL)
...
def draw_info_text(image, brect, handedness, hand_sign_text,
    ...
    # Draw rectangle above bounding box
...
def draw_hand_history(image, hand_history):
    for index in range(len(hand_history)-1):
        p1 = hand_history[index]
        p2 = hand_history[index+1]
        if p1[0] != 0 and p1[1] != 0 and p2[0] != 0 and p2[1] != 0:
            cv.line(image, tuple(p1), tuple(p2),
                (255, 0, 0), 2)
